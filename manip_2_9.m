...
for i_snr = 1:length(Es_N0_dB)
    ...
    for frame = 1:nb_frames
        ...
        % Code 4: EMLDecode
        encoded_bits2_hamming = Encode(G_Hamming, bits);
        symbols_eml = Bit2SymbolMappingQPSKGray(A, encoded_bits2_hamming);
        received_eml = AWGN(Delta, noise_variance, symbols_eml * exp(1i * phi));
        [sym_decoded,decoded_bits_eml] = EMLDecode(A, Delta, list_qpsk_codewords, received_eml);
...
% affichages
figure;
semilogy(Eb_N0_dB_rep_1_2, ber_repetition_1_2, 'b-o', 'LineWidth', 0.5, 'DisplayName', 'Repetition Code x2');
hold on;
semilogy(Eb_N0_dB_eml, ber_eml, 'c-d', 'LineWidth', 1.5, 'DisplayName', 'EML Decode');

semilogy(Eb_N0_dB_rep_1_3, ber_repetition_1_3, 'g-s', 'LineWidth', 0.5, 'DisplayName', 'Repetition Code x3');
semilogy(Eb_N0_dB_hamming, ber_hamming, 'm-^', 'LineWidth', 0.5, 'DisplayName', 'Hamming Code (8,4)');
xlabel('E_b/N_0 (dB)');
ylabel('Taux erreur binaire');
grid on;
legend show;
title('Taux erreur binaire des differents codes en fonction de E_b/N_0');
